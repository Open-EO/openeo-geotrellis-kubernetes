...
export WORK_DIR=/home/sram-cscale_test-jdries/OPENEO
...
export OPENEO_CATALOG_FILES=/home/sram-cscale_test-jdries/OPENEO/layercatalog.json
#export KUBE=false
srun -n 1 -N 1 singularity exec /project/cscale_test/Public/openeo/openeo-yarn_latest.sif /opt/spark3.2.0/bin/spark-submit --master ${MASTER_URL} --executor-memory ${SLURM_SPARK_MEM}M --jars /opt/geotrellis-extensions-static.jar,/opt/geotrellis-backend-assembly-static.jar,/opt/openeo-logging-static.jar  /opt/venv/lib64/python3.8/site-packages/openeogeotrellis/deploy/batch_job.py /home/sram-cscale_test-jdries/OPENEO/job_specification.json /home/sram-cscale_test-jdries/OPENEO out_file log job_metadata.json "1.0.0" "[]" spark false
